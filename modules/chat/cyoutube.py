...
from modules.helper.message import TextMessage
...
class YTReader(threading.Thread):
    def __init__(self, api, live_id, queue):
        self.api = api
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                if self.load_config():
                    self.reader = YTReader(self.chat_module.api, self.live_id, self.queue)
...
class CYoutube(ChatModule):
    ...
    def load_module(self, *args, **kwargs):
        ...
        if self.get_config('api', 'refresh_token').value:
            ...
            self.config['config']['warning'].color = 'red'
